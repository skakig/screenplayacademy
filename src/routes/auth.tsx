...
export const Route = createFileRoute("/auth")({
  ssr: false,
  head: () => ({ meta: [{ title: "Sign in — SceneSmith AI" }] }),
  component: AuthPage,
});
  ...
  const handleEmail = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        toast.success("Account created. Welcome to SceneSmith.");
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 mb-3">
            ...
            <span className="text-2xl font-bold tracking-tight">SceneSmith<span className="text-primary"> AI</span></span>
          </div>
          <p className="text-sm text-muted-foreground">Write the script. See the scene. Hear the table read.</p>
        </div>
        <Card className="p-6 shadow-2xl border-border/50 backdrop-blur">
          <h1 className="text-2xl font-semibold mb-1">{mode === "signin" ? "Welcome back" : "Create your studio"}</h1>
          <p className="text-sm text-muted-foreground mb-6">
            {mode === "signin" ? "Sign in to continue your story." : "Start writing your first screenplay."}
          </p>
          ...
          <div className="relative my-4 text-center">
            <span className="bg-card px-2 text-xs text-muted-foreground relative z-10">or with email</span>
