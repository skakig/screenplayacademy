    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto grid min-h-screen max-w-6xl gap-10 px-4 py-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:py-12">
        <section className="relative overflow-hidden rounded-[2rem] border border-border/60 bg-[linear-gradient(180deg,rgba(247,245,240,0.95),rgba(247,245,240,1))] p-8 shadow-[0_30px_90px_-40px_rgba(15,27,45,0.3)] lg:p-12">
          ...
          <p className="mt-10 text-xs uppercase tracking-[0.24em] text-primary">AI-powered storytelling studio</p>
          ...
          <p className="mt-6 max-w-xl text-lg leading-8 text-foreground/75">
            Write, develop, and refine your work inside a storytelling studio built for
            screenplays, novels, worldbuilding, comedy, and audio storytelling.
          </p>
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            {[
              ...
              <div key={item} className="rounded-2xl border border-border/60 bg-background/80 px-4 py-3 text-sm text-muted-foreground shadow-sm">
                {item}
              </div>
            ))}
          </div>
        ...
        <div className="w-full max-w-md justify-self-center lg:justify-self-end">
          <Card className="border-border/60 bg-card/85 p-6 shadow-[0_24px_80px_-36px_rgba(15,27,45,0.45)] backdrop-blur-sm">
            ...
            <Button type="button" variant="outline" className="mb-4 w-full" onClick={handleGoogle} disabled={loading}>
              <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
              Continue with Google
            </Button>

            <div className="relative my-4 text-center">
              <span className="relative z-10 bg-card px-2 text-xs uppercase tracking-[0.18em] text-muted-foreground">or with email</span>
