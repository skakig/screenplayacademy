...
function ModulePage() {
  ...
  if (!data) return <AppShell><div className="p-10">Module not found.</div></AppShell>;
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        {data.lessons.length === 0 ? (
          <Card className="p-8 text-center border-dashed">
            ...
            <p className="text-sm text-muted-foreground">More lessons in this module are coming soon.</p>
          </Card>
        ) : (
              ...
              return (
                <Link key={l.id} to="/academy/$moduleSlug/$lessonSlug" params={{ moduleSlug, lessonSlug: l.slug }}>
                  <Card className="p-4 hover:border-primary/60 transition cursor-pointer flex items-center gap-3">
                    ...
                    <span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" />{l.estimated_minutes}m</span>
                  </Card>
                </Link>
              );
            })}
